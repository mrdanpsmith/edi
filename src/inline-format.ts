...
// The formatting toolbar's inline buttons (bold/italic/strike/code/link) act on
// the ProseMirror selection by default, but when a spreadsheet cell is the
// active editing target they should format that cell's markdown instead. The
// active table node-view registers itself when it gains focus; the toolbar
// checks the registry before touching the editor selection.

export type InlineCellKind = 'bold' | 'italic' | 'strike' | 'code' | 'link' | 'secret'
